...
function getMedia(c){
 ...
 let el;
 if(c.kind==='video'){el=document.createElement('video');el.src=c.src;el.muted=true;el.playsInline=true;el.preload='auto'}
 else if(c.kind==='image'){el=new Image();el.src=c.src}
 else if(c.kind==='audio'){el=document.createElement('audio');el.src=c.src;el.preload='auto'}
 media.set(c.id,el);return el;
}
...
function renderPreview(){
 ...
 for(const c of activeClips()){
   ...
   ctx.save();ctx.globalAlpha=c.opacity??1;
   if(c.kind==='image'){const im=getMedia(c);if(im?.complete){const r=Math.max(state.w/im.naturalWidth,state.h/im.naturalHeight),dw=im.naturalWidth*r*(c.scale||1),dh=im.naturalHeight*r*(c.scale||1);ctx.translate(state.w/2+(c.x||0),state.h/2+(c.y||0));ctx.rotate((c.rotation||0)*Math.PI/180);ctx.filter=state.effect==='mono'?'grayscale(1)':state.effect==='cinematic'?'contrast(1.1) saturate(.85)':'none';ctx.drawImage(im,-dw/2,-dh/2,dw,dh)}}
   if(c.kind==='video'){const v=getMedia(c);if(v?.readyState>=2){const local=(state.time-c.start)*(c.speed||1);if(Math.abs(v.currentTime-local)>.12)try{v.currentTime=Math.min(local,v.duration||local)}catch{}const r=Math.max(state.w/v.videoWidth,state.h/v.videoHeight),dw=v.videoWidth*r*(c.scale||1),dh=v.videoHeight*r*(c.scale||1);ctx.translate(state.w/2+(c.x||0),state.h/2+(c.y||0));ctx.rotate((c.rotation||0)*Math.PI/180);ctx.filter=state.effect==='mono'?'grayscale(1)':state.effect==='cinematic'?'contrast(1.1) saturate(.85)':'none';ctx.drawImage(v,-dw/2,-dh/2,dw,dh)}}
...
function fitCanvas(){const stage=$('#videoStage'),maxW=Math.max(250,stage.clientWidth-28),maxH=Math.max(280,stage.clientHeight-28),fit=Math.min(maxW/state.w,maxH/state.h);canvas.style.width=`${state.w*fit}px`;canvas.style.height=`${state.h*fit}px`}
function updateTime(){$('#timeLabel').textContent=`${fmt(state.time)} / ${fmt(state.duration)}`;$('#playhead').style.left=`${70+state.time*pxPerSec()}px`}
...
function addText(){push();const c={id:uid(),kind:'text',track:'Text',name:'Title',start:state.time,duration:Math.min(4,state.duration-state.time),text:'YOUR TEXT',x:0,y:0,scale:1,rotation:0,opacity:1,size:110,color:'#ffffff'};state.clips.push(c);state.selected=c.id;sync()}
function addFile(file,trackKind){
...
$('#addVideoText').onclick=addText;$('#addTextClip').onclick=addText;$('#mobileText').onclick=addText;
...
$('#tracks').addEventListener('pointerdown',e=>{const clip=e.target.closest('.clip');if(!clip)return;state.selected=clip.dataset.id;const c=selected();inspect();renderTimeline();const pps=pxPerSec(),x=e.clientX,trim=e.target.dataset.trim;push();drag={c,startX:x,start:c.start,dur:c.duration,trim};clip.setPointerCapture(e.pointerId)});
...
function play(){
 state.playing=!state.playing;$('#playBtn').textContent=state.playing?'❚❚ Pause':'▶ Play';if(!state.playing){cancelAnimationFrame(raf);return}let last=performance.now();const loop=now=>{if(!state.playing)return;const dt=(now-last)/1000;last=now;state.time+=dt;if(state.time>=state.duration){state.time=0;state.playing=false;$('#playBtn').textContent='▶ Play'}renderPreview();renderTimeline();if(state.playing)raf=requestAnimationFrame(loop)};raf=requestAnimationFrame(loop)
}
...
$$('[data-vtab]').forEach(b=>b.onclick=()=>{const n=b.dataset.vtab;$$('[data-vtab]').forEach(x=>x.classList.toggle('active',x===b));$$('[data-vpanel]').forEach(p=>p.classList.toggle('hidden',p.dataset.vpanel!==n))});
$('#videoUndo').onclick=undo;$('#videoRedo').onclick=redo;
$('#videoSave').onclick=()=>{const ps=store.get('mk97.projects',[]);ps.push({type:'Video',name:$('#videoProjectName').value||'MK97 Video',saved:new Date().toLocaleString()});store.set('mk97.projects',ps.slice(-30));bump('projects');toast('Video project entry saved locally')};
...
const bd=$('#videoBackdrop');function close(){ $('#videoLeft').classList.remove('open');$('#videoRight').classList.remove('open');bd.classList.remove('open')}bd.onclick=close;$$('[data-vopen]').forEach(b=>b.onclick=()=>{close();(b.dataset.vopen==='left'?$('#videoLeft'):$('#videoRight')).classList.add('open');bd.classList.add('open')});
