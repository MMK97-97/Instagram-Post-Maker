(() => {
  ...
  class PosterEditor {
    ...
    applyTemplate(id, save = true) {
      ...
      this.state.layers = [
        this.makeTextLayer('Kicker', item.kicker, item.align === 'center' ? 50 : 7, item.titleY - 10, item.align === 'center' ? 82 : item.titleWidth, 25, 900, 'DM Sans', item.palette[2], item.align, 1, 2),
        this.makeTextLayer('Headline', item.title, item.align === 'center' ? 50 : 7, item.titleY, item.titleWidth, item.titleSize, item.font === 'Bebas Neue' ? 400 : 900, item.font, '#ffffff', item.align, .84, 0),
        this.makeTextLayer('Details', item.detail, item.align === 'center' ? 50 : 7, item.titleY + 28, item.align === 'center' ? 78 : 68, 27, 650, 'DM Sans', '#c8cbd0', item.align, 1.15, .4, .82),
        this.makeTextLayer('Footer', item.cta, 7, 91, 45, 19, 900, 'DM Sans', item.palette[2], 'left', 1, 1.8),
        ...preservedImages,
        ...preservedUserLayers
      ];
    ...
    drawImageLayer(ctx, layer, width, height) {
      ...
      ctx.drawImage(image, -boxWidth / 2, -boxHeight / 2, boxWidth, boxHeight);
      layer._bounds = { x: centerX - boxWidth / 2, y: centerY - boxHeight / 2, width: boxWidth, height: boxHeight };
      ctx.restore();
    }
    ...
    addImageLayer(assetId) {
      ...
      const layer = {
        ...
        brightness: 0, contrast: 0, saturation: 0, exposure: 0, grayscale: 0, sepia: 0, blur: 0,
    ...
    advancedAppearanceHtml(layer) {
      ...
      return `
        <div class="inspector-section">
          <div class="micro-label">APPEARANCE</div><h3>Blend</h3>
    ...
    layersHtml() {
      ...
      return `<div class="layer-list">${this.state.layers.slice().reverse().map(layer => {
        ...
        return `
          <div class="layer-row ${active ? 'active' : ''}" data-layer-row="${layer.id}">
    ...
    bindAdvancedInspector(layer) {
      ...
      on('#insTextColor','input',event => { layer.color = event.target.value; this.safeRender(); });
      ...
      on('#insResetPhoto','click',() => {
        layer.brightness = 0; layer.contrast = 0; layer.saturation = 0; layer.exposure = 0; layer.grayscale = 0; layer.sepia = 0; layer.blur = 0; layer.flipX = false; layer.flipY = false;
    ...
    bindUI() {
      ...
      $('#posterMediaInput')?.addEventListener('change',async event => {
        try { await this.importImages(event.target.files); }
      ...
      this.canvas.addEventListener('dblclick',event => {
        const point = this.pointerCoordinates(event);
        const layer = this.hitTest(point.x, point.y);
        if (layer) { this.state.selectedId = layer.id; this.safeRender(); this.renderInspector(); }
      });
