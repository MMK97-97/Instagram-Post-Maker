...
function defaultTemplate(t){
 ...
 state.selected=state.layers[2].id;render();renderLayers();renderInspector();
}
...
function drawSelection(l){
 if(!l||l.visible===false)return;ctx.save();ctx.strokeStyle='#f3c95b';ctx.lineWidth=4;ctx.setLineDash([14,10]);ctx.strokeRect(l.x,l.y,l.w,l.h);ctx.setLineDash([]);for(const [x,y] of [[l.x,l.y],[l.x+l.w,l.y],[l.x,l.y+l.h],[l.x+l.w,l.y+l.h]]){ctx.fillStyle='#fff';ctx.strokeStyle='#111';ctx.lineWidth=3;ctx.beginPath();ctx.arc(x,y,11,0,Math.PI*2);ctx.fill();ctx.stroke()}ctx.restore();
}
function render(){
 ...
 if(state.grid){ctx.save();ctx.strokeStyle='rgba(255,255,255,.06)';ctx.lineWidth=1;for(let x=0;x<state.w;x+=90){ctx.beginPath();ctx.moveTo(x,0);ctx.lineTo(x,state.h);ctx.stroke()}for(let y=0;y<state.h;y+=90){ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(state.w,y);ctx.stroke()}ctx.restore()}
 for(const l of state.layers)drawLayer(l);drawSelection(selected());fitCss();
}
function fitCss(){
 const maxW=Math.max(260,stage.clientWidth-40),maxH=Math.max(320,stage.clientHeight-40),fit=Math.min(maxW/state.w,maxH/state.h);
 const s=fit*state.zoom;canvas.style.width=`${state.w*s}px`;canvas.style.height=`${state.h*s}px`;frame.style.width=`${state.w*s}px`;frame.style.height=`${state.h*s}px`;$('#zoomLabel').textContent=`${Math.round(state.zoom*100)}%`;
}
...
function renderTemplates(){
 ...
 $('#editorTemplateCount').textContent=list.length;
...
$('#editorTemplateSearch').oninput=renderTemplates;$('#editorTemplateGrid').onclick=e=>{const b=e.target.closest('[data-id]');if(!b)return;push();const t=T.find(x=>x.id===b.dataset.id);if(t){defaultTemplate(t);store.set('mk97.selectedTemplate',t.id);bump('templatesUsed')}};
...
$('#layerList').onclick=e=>{const row=e.target.closest('[data-id]');if(!row)return;const l=state.layers.find(x=>x.id===row.dataset.id);const act=e.target.dataset.act;if(act==='vis'){push();l.visible=l.visible===false?true:false;renderLayers();render();return}if(act==='lock'){push();l.locked=!l.locked;renderLayers();return}state.selected=l.id;renderLayers();renderInspector();render()};
...
function renderInspector(){
 ...
 if(!l){box.innerHTML=`<section class="inspector-section"><h4>Canvas</h4>${ctrl('Background',state.bg,'bg','color')}<div class="row2"><button class="small-btn" data-canvas="portrait">Portrait</button><button class="small-btn" data-canvas="story">Story</button></div></section>`;return}
...
$('#uploadImageBtn').onclick=()=>$('#imageInput').click();$('#imageInput').onchange=e=>{const f=e.target.files?.[0];if(!f)return;const r=new FileReader();r.onload=()=>addImage(r.result);r.readAsDataURL(f)};
$('#addTextBtn').onclick=addText;$('#addShapeBtn').onclick=addShape;
$('#removeBgBtn').onclick=()=>{const l=selected();if(!l||l.type!=='image'){toast('Select an image layer first');return}const im=getImage(l.src);if(!im.complete){toast('Image is still loading');return}push();const c=document.createElement('canvas');c.width=im.naturalWidth;c.height=im.naturalHeight;const x=c.getContext('2d',{willReadFrequently:true});x.drawImage(im,0,0);const d=x.getImageData(0,0,c.width,c.height),p=d.data,pts=[[2,2],[c.width-3,2],[2,c.height-3],[c.width-3,c.height-3]];let rr=0,gg=0,bb=0;for(const [px,py] of pts){const i=(py*c.width+px)*4;rr+=p[i];gg+=p[i+1];bb+=p[i+2]}rr/=4;gg/=4;bb/=4;for(let i=0;i<p.length;i+=4){const dist=Math.hypot(p[i]-rr,p[i+1]-gg,p[i+2]-bb)/1.732;if(dist<30)p[i+3]=0;else if(dist<60)p[i+3]=Math.round(p[i+3]*(dist-30)/30)}x.putImageData(d,0,0);l.src=c.toDataURL('image/png');imageCache.delete(l.src);render();toast('Background removed')};
...
$('#deleteBtn').onclick=del;$('#duplicateBtn').onclick=dup;$('#undoBtn').onclick=undo;$('#redoBtn').onclick=redo;
...
function exportPoster(){
 ...
 a.download=`${($('#projectName').value||'MK97-poster').replace(/[^\w-]+/g,'-')}.png`;
...
$('#saveProjectBtn').onclick=()=>{const ps=store.get('mk97.projects',[]);ps.push({type:'Poster',name:$('#projectName').value||'MK97 Poster',saved:new Date().toLocaleString()});store.set('mk97.projects',ps.slice(-30));store.set('mk97.posterState',state);bump('projects');toast('Project saved locally')};
function syncAll(){render();renderLayers();renderInspector()}
$$('.tab').forEach(b=>b.onclick=()=>{const n=b.dataset.tab;$$('.tab').forEach(x=>x.classList.toggle('active',x===b));$$('[data-panel]').forEach(p=>p.classList.toggle('hidden',p.dataset.panel!==n))});
const backdrop=$('#sheetBackdrop');
function closeSheets(){$('#leftPanel').classList.remove('open');$('#rightPanel').classList.remove('open');backdrop.classList.remove('open')}
backdrop.onclick=closeSheets;
$$('[data-open]').forEach(b=>b.onclick=()=>{closeSheets();const p=b.dataset.open==='left'?$('#leftPanel'):$('#rightPanel');p.classList.add('open');backdrop.classList.add('open');if(b.dataset.tabOpen){const tab=$(`.tab[data-tab="${b.dataset.tabOpen}"]`);tab?.click()}});
...
renderTemplates();
const chosen=T.find(x=>x.id===store.get('mk97.selectedTemplate','match-day'))||T[0];defaultTemplate(chosen);
